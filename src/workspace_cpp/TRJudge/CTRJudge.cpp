...
{
	if (m_args.size() != 7 && m_args.size() != 4)
		return false;
	if(m_args[1] == "-j")
	{
		if(m_args.size() != 7 ||
...
static void getFilesByLeftKey(const vector<string>& in, vector<string>& out, const char* key)
{
	for(size_t i = 0; i < in.size(); ++i)
	{
		...
		const string filename = getFileName(filepath);
...
{
	// remove dir
	string commandRMDir = string("rm -rf \"") + m_dstDir + "\"";
#ifdef _DEBUG
		cout<<"command: "<<commandRMDir<<endl;
#endif
	system(commandRMDir.c_str());
	// make dir
	mkpath(m_dstDir.c_str());
	...
	for(size_t i = 0; i < m_vsuiteReports.size(); ++i)
	{
		...
		string command = string("cp -r ") + "\"" + szSrc + "\" \"" + szDst +  "\"";
#ifdef _DEBUG
		cout<<"command: "<<command<<endl;
#endif
		system(command.c_str());
		lfTotalLine += sr.lfTotalLine;
		lfLineCov += sr.lfLineCoverage;
	}
	...
	string infoFilePath = m_dstDir + "/CovRep.txt";
	remove(infoFilePath.c_str());
	writeLineToFile(infoFilePath, string(s.str()));
	return m_vsuiteReports.size();
}
