...
int CDataTest::executeATest(const std::string& className)
{
	...
	string repoPath_EmmaTest = m_outputReportDir + "/report@" + className + ".txt";
	...
	*/
	...
	for(int i = 0; i < 3; ++i)
	{
		if(systemcmd(command, 20))
			return 0;
	}
	return 1;
	//return system(commandRenameCoverageReport.c_str());
}
