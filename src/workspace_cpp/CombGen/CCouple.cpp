...
CCouple::CCouple(const std::string& m1Dir, const std::string& m2Dir,
		const std::string& outputDir,int nReproduceRate, int nBeginIndex, int seed) :
m_m1Dir(m1Dir),
m_m2Dir(m2Dir),
...
m_nSeed(seed),
m_countCases1Usage(-1),
m_countCases2Usage(-1)
{
...
void CCouple::saveChildIdentity(const string& outputDir)
{
	...
	string fileSave = outputDir + "/CombGen-ID/Identity.txt";
	// create directory
	mkpath(getDirectory(fileSave).c_str(), S_IRUSR| S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IROTH);
	// write file
	ofstream outputFile;
	outputFile.open(fileSave.c_str(), ofstream::out);
	if(!outputFile.is_open())
		return;
	outputFile<<"parent1: " << m_m1Dir << endl;
	outputFile<<"parent2: " << m_m2Dir << endl;
	outputFile<<"index: " << m_nBeginIndex << endl;
	outputFile<<"crossing seed: " << m_nSeed << endl;
	outputFile<<"gen1: " << m_countCases1Usage << "/" << totalCases << endl;
	outputFile<<"gen2: " << m_countCases2Usage << "/" << totalCases << endl;
	outputFile.close();
}
