...
void CCombGen::showCommands()
{
	...
	cout<<"2) CombGen -m <testSuiteDir1> <testSuiteDir2> <reproduction rate (1-20)> [<seed>] [<beginIndex>]"<<endl;
}
...
bool CCombGen::parseCommands()
{
	...
	{
		m_mode = GROUP;
		m_strGroupDir = fixpath(m_args[2]);
...
static vector<SCOUPLE> makeCouples(const vector<string>& childDir, int nMateRate)
{
	...
	if(nDirNum >= 2)
	{
		...
		for(size_t i = 0; i < childDir.size(); ++i)
		{
			INDEXED_CHOICE one;
			one.index = i;
			one.path = childDir[i];
